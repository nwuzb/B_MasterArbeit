...
本研究的核心贡献在于提出并验证了一种创新的5维物理约束卡尔曼滤波运动模型（Phys-5D）。通过将相机的透视几何作为强先验，并引入符合物理直觉的运动约束，我们的Phys-5D模型在关键的应用指标上显著超越了传统的纯运动学模型。实验结果表明，与标准的8维匀速模型（CV-8D）和更复杂的12维加速度模型（CA-12D）相比，Phys-5D模型在保持高跟踪精度的同时，极大地增强了身份维持的稳定性，最终在自建的MOT-RPCH数据集上实现了仅为2.24\%的平均绝对百分比计数误差。
