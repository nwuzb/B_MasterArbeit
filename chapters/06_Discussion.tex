...
Our final system—integrating a domain-adapted YOLOv11m detector, a custom-trained EfficientNet-B0 Re-ID model, and an innovative 5D physics-constrained Kalman filter (Phys-5D)—achieves excellent performance on application-level metrics. On our MOT-RPCH benchmark, the mean absolute percentage error (MAPE) of the final count is only 2.24\%. This starkly contrasts with the 14.59\% MAPE of the baseline 8D constant-velocity model (CV-8D) and the 6.99\% MAPE of the 12D constant-acceleration model (CA-12D), forming the basis for the deeper discussion that follows.
