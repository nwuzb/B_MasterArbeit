...
This chapter provides an in-depth and systematic analysis of the experimental results presented in Chapter 5. Rather than merely listing numbers, we investigate the fundamental reasons behind the performance differences among models, and elucidate why the proposed physics-constrained model (Phys-5D) is superior to traditional purely kinematic models. We further analyze how the system’s components (detector, re-identification model, and counting strategy) interact synergistically, and discuss the theoretical implications, practical value, and inherent limitations of this work, laying a solid foundation for the concluding remarks and future directions.

6.1 Comprehensive Interpretation of Results and Core Findings

As stated in Chapter 1, the overarching goal of this work is to build a high-accuracy, high-robustness, real-time crowd detection, tracking, and counting system tailored to the highly challenging dynamics on railway platforms. The experiments in Chapter 5 provide solid quantitative evidence of the effectiveness of our approach. Taken together, the results point to a core finding: for scenarios with strong physical priors, incorporating domain knowledge (e.g., camera geometry) into the motion model is far more effective and robust than merely increasing kinematic model complexity.

Our final system—integrating a domain-adapted YOLOv11m detector, a custom-trained EfficientNet-B0 Re-ID model, and an innovative 5D physics-constrained Kalman filter (Phys-5D)—achieves excellent performance on application-level metrics. On our MOT-TPCH benchmark, the mean absolute percentage error (MAPE) of the final count is only 2.24%. This starkly contrasts with the 14.59% MAPE of the baseline 8D constant-velocity model (CV-8D) and the 6.99% MAPE of the 12D constant-acceleration model (CA-12D), forming the basis for the deeper discussion that follows.

An important observation is that application performance (counting accuracy) is not linearly correlated with traditional MOT metrics (e.g., MOTA). CA-12D and Phys-5D obtain nearly identical MOTA (67.54% vs 67.21%), suggesting similar per-frame detection–association accuracy. However, this obscures a critical difference: the number of identity switches (IDSW) is much higher for CA-12D (avg 39.15) than for Phys-5D (avg 24.80). Identity switches measure how well a tracker preserves identity consistency. For cumulative tasks like counting, which require long-term, stable, and unique identities, trajectory stability and persistent identity are more important than single-frame localization accuracy. Each identity switch can cause the same person to be mistaken for two different identities, inducing overcounts or undercounts. Hence, although CA-12D can fit instantaneous motion better, its weaker identity consistency directly leads to much higher counting errors than Phys-5D. This highlights that application-oriented evaluations must emphasize identity-consistency metrics such as IDF1 and IDSW. Phys-5D’s true advantage lies not only in accurate tracking but also in sustaining consistent identities.

6.2 Motion Models: From Pure Kinematics to Physics-Constrained Modeling and the Trade-offs
...
6.2.1 Limitations of the Baseline (CV-8D)

As the standard baseline in MOT, the 8D constant-velocity (CV-8D) model is our starting point. It assumes linear, constant-speed motion in the image plane. In our scenario—platform pedestrians observed from a moving train—this assumption is fundamentally violated. As the train decelerates, perspective projection causes apparent nonlinear acceleration of pedestrians in the image plane and a nonlinear enlargement of their bounding boxes.
The experimental evidence confirms this mismatch. As shown in \ref{tab:CV8DResults}, CV-8D exhibits many misses (avg 645) and a high counting error (MAPE 14.59%). The root cause is that during occlusions, the constant-velocity Kalman filter cannot predict the object’s reappearance position and size accurately. Large deviations between prediction and observation (i.e., large Mahalanobis distance) lead to association failures: the tracker either terminates the original trajectory (misses) or allocates a new identity upon reappearance (ID switches). The performance bottleneck is thus the contradiction between the model’s kinematic assumptions and the physical reality of the scene.

6.2.2 Gains and Losses of the Higher-Order Kinematic Model (CA-12D)

A straightforward improvement to CV-8D is to raise the kinematic order by adding accelerations. The 12D constant-acceleration (CA-12D) model embodies this idea and can, in principle, fit nonlinear motion trajectories.
...
6.2.3 Why the Physics-Constrained Model (Phys-5D) Prevails
...
\begin{table}[!ht]
    ...
    \end{tabular}
    \caption{Three motion models: counting performance comparison}
...
6.3 Synergy and Contributions of System Components
...
Dataset coverage: Our MOT-TPCH is constructed primarily from public videos and may lack extreme conditions (night, rain/snow, harsh lighting), leaving generalization to out-of-distribution scenarios insufficiently validated.
...
本章旨在对第五章呈现的实验结果进行深入、系统的剖析与解读。讨论将超越简单的数值罗列，深入探究不同模型性能差异背后的根本原因，阐明本研究提出的物理约束模型（Phys-5D）相较于传统运动学模型的优越性。此外，本章还将分析系统中各个组件（检测器、重识别模型、计数策略）的协同效应，并探讨本研究的理论意义、实践价值及其固有的局限性，为最终的结论与未来展望奠定坚实的理论基础。

6.1 实验结果的综合解读与核心发现

本研究的核心目标，如第一章所述，是为铁路站台这一极具挑战性的动态场景，构建一个高精度、高鲁棒性的实时人群检测、跟踪与计数系统。第五章的实验结果为我们评估所提出方法的有效性提供了坚实的量化依据。综合来看，实验数据清晰地指向一个核心发现：在处理具有强物理先验的特定场景时，将领域知识（如相机几何）融入运动模型，是一种远比单纯增加模型运动学复杂度更为有效和稳健的技术路径。

本次研究的最终成果：一个集成了经过领域自适应微调的YOLOv11m检测器、定制化训练的EfficientNet-B0重识别（Re-ID）模型以及创新的5维物理约束卡尔曼滤波器（Phys-5D）的跟踪计数系统在关键的应用层指标上取得了卓越的性能。在自建的MOT-TPCH测试集上，该系统的平均绝对百分比误差（MAPE）仅为2.24\%。这一高精度的计数结果，与作为基准的标准8维匀速模型（CV-8D）高达14.59\%的MAPE，以及引入加速度项的12维模型（CA-12D）的6.99\%的MAPE形成了鲜明对比。这一性能层级上的显著差异，构成了本章后续深入讨论的基石。
...
如表 \ref{tab:3modelsCountingComparisonResults}所示，三种运动模型关键性能指标对比
...
数据集的局限性：我们自建的MOT-TPCH数据集是本次研究能够成功进行的关键 。然而，该数据集主要来源于公开视频，尽管我们尽力使其多样化，但其在覆盖所有真实世界变化方面仍有不足，例如，缺乏夜间、雨雪等恶劣天气和极端光照条件下的数据。因此，模型在这些未曾见过的、分布外（out-of-distribution）场景下的泛化性能尚未得到充分验证。
